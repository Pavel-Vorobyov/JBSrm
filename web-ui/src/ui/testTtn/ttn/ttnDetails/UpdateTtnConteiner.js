...
class UpdateTtnConteiner extends Component {
    ...
    componentDidMount() {
        let ttnId = this.props.match.params.id;
        axios.get('/api/ttns/' + ttnId)
        .then(response => {
            this.setState({
                ...this.state,
                ttnDto: response.data,
            })
        });
    }
    ...
    handleSaveButton() {
        if (this.isValidDto()) {
            axios.put('/api/ttns/' + this.state.ttnDto.id, {
                id: this.state.ttnDto.id,
                createAt: this.state.ttnDto.createAt,
                createdBy: this.state.ttnDto.createdBy,
                deleted: this.state.ttnDto.deleted,
                driverId: this.state.ttnDto.driver.id,
                products: this.state.ttnDto.products,
                transportId: this.state.ttnDto.transport.id,
                ttnState: this.state.ttnDto.ttnState,
                waybill: this.state.ttnDto.waybill,
            })
            .then(response => {
    ...
    handleProductDetailsSearch = isOpnen => {
        if (typeof this.state.ttnDto.transport.bodyType === 'undefined') {
