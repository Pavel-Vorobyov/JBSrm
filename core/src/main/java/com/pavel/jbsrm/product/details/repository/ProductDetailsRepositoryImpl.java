...
public class ProductDetailsRepositoryImpl extends QuerydslRepositorySupport implements ProductDetailsRepositoryCustom {
    private String queryStart = "select id, title, description, price, required_type, deleted from product_details " +
            "where as_tsvector(as_text(id), as_text(required_type), title, as_text(deleted)) @@ to_tsquery('";
