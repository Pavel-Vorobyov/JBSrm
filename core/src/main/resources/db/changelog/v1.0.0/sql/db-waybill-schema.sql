...
(
    ...
    check_point_id serial NOT NULL
);

CREATE INDEX IF NOT EXISTS idx_fts_waybill_checkpoints ON waybill_checkpoints
  USING gin(as_tsvector(id::TEXT, waybill_id::TEXT, check_point_id::TEXT));

CREATE INDEX IF NOT EXISTS idx_fts_check_point ON check_point
  USING gin(as_tsvector(as_text(id), title, as_text(check_point_status), as_text(deleted)));
...
insert into check_point (title, lat, lng, check_point_status,deleted)
	values ('check+point', 19.618997640855298, -83.6948828125, 'PASSED', false);
insert into check_point (title, lat, lng,check_point_status,deleted)
	values ('check+point', 20.618997640855298, -84.6948828125, 'PASSED', false);
...
insert into waybill_checkpoints (waybill_id, check_point_id) values (1, 1);
insert into waybill_checkpoints (waybill_id, check_point_id) values (1, 2);